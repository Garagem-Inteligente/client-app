...
class FipeApiService {
  ...
  async getModelsByBrand(
    ...
    const queryParam = reference ? `?reference=${reference}` : '';
    return this.fetch<FipeModel[]>(`/${vehicleType}/brands/${brandId}/models${queryParam}`);
  }
   ...
   */
  async searchBrands(vehicleType: FipeVehicleType, searchTerm: string): Promise<FipeBrand[]> {
  ...
  async searchModels(
    ...
    const normalizedSearch = searchTerm.toLowerCase().trim();
    return models.filter((model) => model.name.toLowerCase().includes(normalizedSearch));
  }
}
