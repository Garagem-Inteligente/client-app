        ...
        <div class="modern-stats-grid">
          ...
          <div class="modern-stat-card gradient-success">
            ...
            <div class="stat-content">
              ...
              <div class="stat-body">
                <h3 class="stat-number">{{ formatCurrency(vehiclesStore.totalMaintenanceCost) }}</h3>
          ...
          <div v-if="vehiclesStore.overdueMaintenance.length > 0" class="modern-stat-card gradient-danger">
            <div class="stat-background"></div>
            <div class="stat-content">
              <div class="stat-header">
        ...
        <div v-if="costOverTimeData.length > 0" class="chart-card">
          ...
          <div class="chart-content">
            <div class="timeline-chart">
              <div 
                v-for="month in costOverTimeData" 
                :key="month.label"
                class="timeline-item"
              >
          ...
          <div v-if="vehiclesStore.vehicles.length > 0" class="vehicle-select-container">
            ...
            >
              ...
              <option 
                ...
                :value="vehicle.id"
              >
                {{ vehicle.make }} {{ vehicle.model }} ({{ getVehicleMaintenanceCount(vehicle.id) }})
              </option>
            </select>
          </div>
        </div>
...
const getVehicleMaintenanceCount = (vehicleId: string): number => {
  return vehiclesStore.maintenanceRecords.filter(record => record.vehicleId === vehicleId).length
}
...
/* Chart Bar Colors */
.chart-bar-color-0 { background: linear-gradient(90deg, #3b82f6 0%, #2563eb 100%); }
.chart-bar-color-1 { background: linear-gradient(90deg, #10b981 0%, #059669 100%); }
.chart-bar-color-2 { background: linear-gradient(90deg, #f59e0b 0%, #d97706 100%); }
.chart-bar-color-3 { background: linear-gradient(90deg, #ef4444 0%, #dc2626 100%); }
.chart-bar-color-4 { background: linear-gradient(90deg, #8b5cf6 0%, #7c3aed 100%); }
.chart-bar-color-5 { background: linear-gradient(90deg, #ec4899 0%, #db2777 100%); }
