<template>
  <div class="searchable-select-fipe">
    ...
    <div
      v-if="showDropdown && filteredOptions.length === 0 && searchQuery && !loading"
      class="no-results"
    >
...
<script setup lang="ts">
  ...
  const handleFocus = () => {
    showDropdown.value = true;
    // Select all text when focusing to make it easier to change selection
    if (inputRef.value && searchQuery.value) {
