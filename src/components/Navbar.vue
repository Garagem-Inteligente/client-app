...
const mainWorkshopItems = [
  ...
  { name: 'Ordens', path: '/workshop/job-orders', icon: 'M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2' }
]

const moreWorkshopItems = [
  { name: 'Avaliações', path: '/workshop/reviews', icon: 'M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z' }
]

// Computed
const mainItems = computed(() => authStore.isWorkshop ? mainWorkshopItems : mainUserItems)
const moreItems = computed(() => authStore.isWorkshop ? moreWorkshopItems : moreUserItems)
const allItems = computed(() => [...mainItems.value, ...moreItems.value])

const dashboardPath = computed(() => authStore.isWorkshop ? '/workshop/dashboard' : '/dashboard')
const profilePath = computed(() => authStore.isWorkshop ? '/workshop/profile' : '/profile')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-16">
        ...
        <div class="flex items-center">
          <router-link :to="dashboardPath" class="flex items-center space-x-2">
        ...
        <div class="hidden md:flex md:items-center md:space-x-1">
          ...
          <router-link
            ...
            :to="item.path"
            :class="[
              'px-3 py-2 rounded-lg text-sm font-medium transition-colors duration-200 flex items-center space-x-1.5',
          ...
          <div class="relative">
            <button
              ...
              :class="[
                'px-3 py-2 rounded-lg text-sm font-medium transition-colors duration-200 flex items-center space-x-1',
                menuDropdownOpen || moreItems.some(item => isActivePath(item.path))
                  ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400'
                  : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'
              ]"
            >
        ...
        <div class="flex items-center space-x-3">
          ...
          <div class="hidden md:block relative">
            ...
            >
              ...
              <div class="py-1">
                ...
                <button
                  @click="handleLogout"
                  class="w-full flex items-center space-x-2 px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-gray-100 dark:hover:bg-gray-700"
                >
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button
            ...
            class="md:hidden inline-flex items-center justify-center p-2 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"
          >
    ...
    <div v-if="mobileMenuOpen" class="md:hidden border-t border-gray-200 dark:border-gray-700">
      <div class="px-2 pt-2 pb-3 space-y-1">
        <router-link
          ...
          :class="[
            'block px-3 py-2 rounded-lg text-base font-medium flex items-center space-x-2',
            isActivePath(item.path)
              ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400'
              : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'
          ]"
      ...
      <div class="pt-4 pb-3 border-t border-gray-200 dark:border-gray-700">
        <div class="flex items-center px-5 py-3">
          ...
          <div class="ml-3">
            <div class="text-base font-medium text-gray-900 dark:text-white">{{ authStore.userName }}</div>
            <div class="text-sm text-gray-500 dark:text-gray-400">{{ authStore.userEmail }}</div>
          </div>
        </div>
        <div class="px-2 space-y-1">
          <router-link
            :to="profilePath"
            @click="mobileMenuOpen = false"
            class="flex items-center space-x-2 px-3 py-2 rounded-lg text-base font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"
          >
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
            </svg>
          ...
          <button
            @click="handleLogout"
            class="w-full flex items-center space-x-2 px-3 py-2 rounded-lg text-base font-medium text-red-600 dark:text-red-400 hover:bg-gray-100 dark:hover:bg-gray-700"
          >
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
            </svg>
